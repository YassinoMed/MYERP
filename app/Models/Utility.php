...
class Utility extends Model
{
    ...
    public static function settings()
    {
        ...
        $settings = [
            ...
            "cust_theme_bg" => "on",
            "cust_darklayout" => "off",
            "cust_darklayout_auto" => "off",
            "color" => "",
            "SITE_RTL" => "off",
            "module_color_default" => "",
            "module_color_account" => "",
            "module_color_crm" => "",
            "module_color_hrm" => "",
            "module_color_project" => "",
            "module_color_pos" => "",
            "purchase_prefix" => "#PUR",
    ...
    public static function languages()
    {
        if (self::$languageSetting == null) {
            $languages = collect(Utility::langList());
    ...
    public static function dispatchAutomationEvent(string $event, $model, ?int $createdBy = null): void
    {
        $ownerId = $createdBy;
        if ($ownerId === null && \Auth::check()) {
        ...
        $payload = [
            ...
            'data' => is_object($model) && method_exists($model, 'toArray') ? $model->toArray() : $model,
        ];

        try {
            $webhooks = \App\Models\Integrations\Webhook::where('user_id', $ownerId)->get();
        ...
        try {
            $zapierHooks = \App\Models\Integrations\ZapierHook::where('user_id', $ownerId)->where('event', $event)->get();
            foreach ($zapierHooks as $zapierHook) {
                $zapierHook->trigger($payload);
            }
        } catch (\Throwable $th) {
        }
        ...
        try {
            if (is_object($model)) {
                ...
                $workflows = \App\Models\Workflow::where('created_by', $ownerId)
                    ->whereIn('trigger_model', $triggerModels)
