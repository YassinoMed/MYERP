...
class AuthenticatedSessionController extends Controller
{
    ...
    public function store(LoginRequest $request)
    {
        ...
        if($user->type != 'company' && $user->type != 'super admin')
        {
            ...
            if (!app()->environment('testing') && !empty($ip)) {
                $query = [];

                $raw = @file_get_contents('http://ip-api.com/php/' . $ip);
                $unserialized = is_string($raw) ? @unserialize($raw) : null;
                ...
                $whichbrowser = null;
                if ($userAgent !== '' && class_exists(\WhichBrowser\Parser::class)) {
                    $whichbrowser = new \WhichBrowser\Parser($userAgent);
                }

                if ($whichbrowser && ($whichbrowser->device->type ?? null) === 'bot') {
                    return;
                }

                $referrer = $request->headers->get('referer');
                $referrerParsed = $referrer ? parse_url($referrer) : null;

                $query['browser_name'] = $whichbrowser ? ($whichbrowser->browser->name ?? null) : null;
                $query['os_name'] = $whichbrowser ? ($whichbrowser->os->name ?? null) : null;
                $query['browser_language'] = $request->getPreferredLanguage() ? mb_substr($request->getPreferredLanguage(), 0, 2) : null;
                $query['device_type'] = function_exists('get_device_type') ? get_device_type($userAgent) : null;
                $query['referrer_host'] = !empty($referrerParsed['host']) ? $referrerParsed['host'] : null;
                $query['referrer_path'] = !empty($referrerParsed['path']) ? $referrerParsed['path'] : null;
