    ...
    <div class="all-button-box row d-flex justify-content-end">
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('create estimation')): ?>
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#" data-url="<?php echo e(route('estimations.create')); ?>" data-size="sm" data-ajax-popup="true" data-title="<?php echo e(__('Create Estimate')); ?>" class="btn btn-xs btn-white btn-icon-only width-auto"><i class="ti ti-plus"></i> <?php echo e(__('Create')); ?></a>
            </div>
        <?php endif; ?>
    </div>
<?php $__env->stopSection(); ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if($estimate->status == 0): ?>
                                            <span class="badge badge-pill status_badge badge-primary"><?php echo e(__(\App\Models\Estimation::$statues[$estimate->status])); ?></span>
                                        <?php elseif($estimate->status == 1): ?>
                                            <span class="badge badge-pill status_badge badge-danger"><?php echo e(__(\App\Models\Estimation::$statues[$estimate->status])); ?></span>
                                        <?php elseif($estimate->status == 2): ?>
                                            <span class="badge badge-pill status_badge badge-warning"><?php echo e(__(\App\Models\Estimation::$statues[$estimate->status])); ?></span>
                                        <?php elseif($estimate->status == 3): ?>
                                            <span class="badge badge-pill status_badge badge-success"><?php echo e(__(\App\Models\Estimation::$statues[$estimate->status])); ?></span>
                                        <?php elseif($estimate->status == 4): ?>
                                            <span class="badge badge-pill status_badge badge-info"><?php echo e(__(\App\Models\Estimation::$statues[$estimate->status])); ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <?php if(Auth::user()->type != 'client'): ?>
                                        <td class="Action">
                                            <span>
                                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('view estimation')): ?>
                                                    <a href="<?php echo e(route('estimations.show',$estimate->id)); ?>" class="edit-icon bg-warning"> <i class="ti ti-eye"></i></a>
                                                <?php endif; ?>
                                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('edit estimation')): ?>
                                                    <a href="#" data-url="<?php echo e(URL::to('estimations/'.$estimate->id.'/edit')); ?>" data-ajax-popup="true" data-title="<?php echo e(__('Edit Estimation')); ?>" class="edit-icon" data-toggle="tooltip" data-original-title="<?php echo e(__('Edit')); ?>"><i class="ti ti-pencil text-white"></i></a>
                                                <?php endif; ?>
                                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('delete estimation')): ?>
                                                    <a href="#" class="delete-icon" data-toggle="tooltip" data-original-title="<?php echo e(__('Delete')); ?>" data-confirm="<?php echo e(__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')); ?>" data-confirm-yes="document.getElementById('delete-form-<?php echo e($estimate->id); ?>').submit();"><i class="ti ti-trash"></i></a>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/mohamedyassine/Downloads/Nouveau dossier contenant des éléments 5/erpgosaas-81nulled/codecanyon-33263426-erpgo-saas-all-in-one-business-erp-with-project-account-hrm-crm/main-file/resources/views/estimations/index.blade.php ENDPATH**/ ?>